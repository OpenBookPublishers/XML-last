...
if not os.path.exists(inputfolder):
    shutil.copytree(oebpsfolder, inputfolder, symlinks=False, ignore=shutil.ignore_patterns('*.opf', 'back*', '*cover*', 'half*', 'toc*', 'css', 'font'))
else:
...
if not os.path.exists(XMLfolder):
    shutil.copytree(imagefolder, XMLfolder, symlinks=False, ignore=shutil.ignore_patterns('*.xhtml'))
else:
...
if os.path.exists(audiofolder):
    if not os.path.exists(XMLaudiofolder):
        shutil.copytree(audiofolder, XMLaudiofolder, symlinks=False, ignore=shutil.ignore_patterns('*.xhtml'))
    else:
...
if os.path.exists(videofolder):
    if not os.path.exists(XMLvideofolder):
        shutil.copytree(videofolder, XMLvideofolder, symlinks=False, ignore=shutil.ignore_patterns('*.xhtml'))
    else:
